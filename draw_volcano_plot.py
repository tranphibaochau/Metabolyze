...
def draw_volcano_plot(input_file, significance_threshold=0.05, fold_change_threshold_lower=-0.5, fold_change_threshold_upper=0.5, opacity=1.0):
    ...
    col_log2_foldchange = [c for c in df.columns if c.endswith("Log2FoldChange")] # determine the Log2FoldChange columns
    ...
    with open(f"{os.getcwd()}/output/iframe/{title}.html", "w+") as f:
        for i, c in enumerate(col_ttest_pval):
            ...
            if len(col_ttest_pval) > 1:
                ...
                new_df.rename(columns={c: "ttest_pval", "".join([c.split("_ttest")[0], "_Log2FoldChange"]): "Log2FoldChange"}, inplace=True)
            ...
            plotly.offline.plot(volcano_plot, filename=f"{os.getcwd()}/output/volcano_plot/{title}.html", auto_open=False)
